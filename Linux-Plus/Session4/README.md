...
- Create a `text` file named `linuxway.txt`.
​
```bash
cat << EOF > linuxway.txt
linuxway:Road to reinvent yourself.
EOF
```

- Display the content of linuxway.txt.
​
```bash
cat linuxway.txt
```

- In the content of linuxway.txt, replace or translate aer letters with 'QAZ'.
​
```bash
cat linuxway.txt | tr aer QAZ
```
​
...
```
​
- Delete all the vowels in the content of linuxway.txt.
​
```bash
cat linuxway.txt | tr -d aeiou
```
​
- Write the whole content of linuxway.txt in capital letters.
​
```bash
cat linuxway.txt | tr [a-z] [A-Z]
```
...
>**$?**
​
- This control operator is used to check the status of last executed command. If status shows '0' then command was successfully executed and if shows '1' then command was a failure (we might also get different codes other than 1).
...
​
- Display days.txt or write 'linuxway' on the screen, then write 'one'.
​
```bash
cat days.txt || echo linuxway ; echo one
```
...
​
  ...
  1.a. Search for “linuxway.doc” in the current directory
  ...
  2.Create a file named “linuxway.doc” that contains “Congratulations”
